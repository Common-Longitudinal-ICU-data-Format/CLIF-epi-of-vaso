...
| `NE_WINDOW_HOURS` | No | `24` | NE start (t=0) must fall within ±this many hours of the infection anchor (abx/blood-culture event). Two-sided and inclusive |
| `MIN_NE_RECORDS` | No | `2` | Minimum NE administration records |
| `SOFA_THRESHOLD` | No | `2.0` | Retained for compatibility; **not** used as an inclusion criterion |
| `LACTATE_THRESHOLD` | No | `2.0` | Minimum lactate (mmol/L) — Sepsis-3 requires `>`, Rhee `>=` |
