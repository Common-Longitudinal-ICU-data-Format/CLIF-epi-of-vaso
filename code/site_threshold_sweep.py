...
def _draw_sweep_panel(ax, t, r, lbl, fontsize_title=8, fontsize_ax=8, fontsize_tick=7, fontsize_legend=6):
    ...
    # Vertical line: training-at-risk kappa threshold (matches outcome analysis)
    if "kappa_train_thresh" in r:
        ...
        vline_dir = r["opt_dir"]
    ax.axvline(vline_val, color="black", lw=1.2, ls=":", label=f"κ-train {vline_dir} {vline_val:.2g}")
    ax.axhline(0, color="gray", lw=0.5, ls="--")
    ax.set_xlabel(lbl, fontsize=fontsize_ax)
    ax.set_ylabel("Cohen's Kappa", fontsize=fontsize_ax)
    if "kappa_train_kappa" in r:
        kappa_str = (f"κ-train={r['kappa_train_kappa']:.3f}  "
                     f"Agr={r.get('kappa_train_agree', r['agree_at_opt']):.2%}")
    else:
        kappa_str = f"κ*={r['opt_kappa']:.3f}  Agr={r['agree_at_opt']:.2%}"
    ...
    for col, lbl, r in sweep_results:
        ...
        for t, kge, kle, age, ale in zip(
            ...
            rows.append({
                ...
                "opt_dir":            r["opt_dir"],
                "kappa_train_thresh": round(float(r["kappa_train_thresh"]), 4) if "kappa_train_thresh" in r else None,
                "kappa_train_dir":    r.get("kappa_train_dir"),
                "kappa_train_kappa":  round(float(r["kappa_train_kappa"]), 4) if "kappa_train_kappa" in r and np.isfinite(r["kappa_train_kappa"]) else None,
            })
...
def build_summary_table(sweep_results: list, q0: np.ndarray, q1: np.ndarray,
    ...
    for col, lbl, r in sweep_results:
        ...
        k_val = r.get("kappa_train_kappa", r["opt_kappa"])
        ...
        rows.append({
            ...
            "kappa":        round(k_val, 4) if np.isfinite(k_val) else None,
...
def main():
    ...
    for _col, _lbl, _is_bin in ANALYSIS_FEATURES:
        ...
        if _res is not None:
            ...
            print(f"  {_col:<18}: {_res[1]} {_res[0]:.4g}  κ-train={_res[2]:.3f}")
        else:
    ...
    for col, lbl, is_binary in ANALYSIS_FEATURES:
        ...
        if r is not None:
            kts = (f"  κ-train={r['kappa_train_kappa']:.3f}"
                   if "kappa_train_kappa" in r else "")
            print(f"  {col:<18}: AUROC={r['auroc']:.3f}  k*={r['opt_kappa']:.3f}{kts}")
